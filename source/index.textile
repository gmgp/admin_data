...
You can also define security at a model level. Let's say that to some users you don't want to show any data related to modle <tt>Payment</tt>. In that case following configuration will prevent the user from accessing that information. <tt>@klass</tt> refers to the model class in question and you can perform any operation on that class.

<pre>
AdminData::Config.set = {
  :is_allowed_to_view_klass => lambda {|controller| controller.instance_variable_get('@klass').name == 'Payment' }
}
</pre>

h3. How to use this plugin with rails3

This plugin works with rails3. This plugin has been converted into a gem for usage in <tt>rails 3</tt>.

Since this plugin depends on <tt>will_paginate</tt> gem, first you need to install a version of <tt>will_paginate</tt> that works with rails 3. That can be done by adding following line to <tt>Gemfile</tt> .

<ruby>
gem 'will_paginate', :git => 'git://github.com/mislav/will_paginate.git', :branch => 'rails3'
</ruby>

You can install the gem using bundler like this.
